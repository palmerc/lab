...
my $word;
my (%senseCount, %contextCount, %senseTotals, %vocabulary);
my $baseCorrect = 0;
...
foreach my $instance (keys %{$trainData->{'instance'}}) {
	...
	foreach my $value (@firstArray) {
		$contextCount{$sense}{$value} += 1;
		$vocabulary{$value} += 1;
	}
	foreach my $value (@secondArray) {
		$contextCount{$sense}{$value} += 1;
		$vocabulary{$value} += 1;
	}
...
my %counter;
foreach my $sense (keys %contextCount) {
	foreach my $value (keys %{$contextCount{$sense}}) {
		$counter{$value}++;
	}
}
...
foreach my $sense (keys %contextCount) {
        foreach my $value (keys %{$contextCount{$sense}}) {
		if ($counter{$value} > 1) {
			delete $contextCount{$sense}{$value};
		}
	}
}
...
foreach my $instance (keys %{$testData->{'instance'}}) {
	...
	my @sentence = (@firstArray, @secondArray);
	my %bias;
	foreach my $value (@sentence) {
		foreach my $sense (keys %contextCount) {
			if (exists $contextCount{$sense}{$value}) {
				$bias{$sense} += 1;
			}
		}
	}
	...
	foreach my $sense (keys %senseCount) {
		...
		$sumProb += log(($senseTotals{$sense}{'count'}) / ($senseSum))/log(2) - $bias{$sense};
		$sumProb = abs($sumProb);
		if ($winner < $sumProb) {
			$winner = $sumProb;
			$winningSense = $sense;	
...
print "Context: " . $correctTotal / $totalTotal * 100 . "%\n";
...
foreach my $sense (keys %contextCount) {
	print "$sense:\n";
	foreach my $value (keys %{$contextCount{$sense}}) {
		print "\t$value $contextCount{$sense}{$value}\n";
	}
}
