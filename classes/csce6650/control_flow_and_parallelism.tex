...
\section*{Processors}
The modern processor, von Neumann architecture, is broken into several distinct phases. These phases are Instruction Fetch (IF), Instruction Decode (ID), Execute (EX), and Write Back (WB). An instruction is taken from the queue of instructions to be executed, This structure imposes a certain ordering on the way instructions are executed. The simplistic version of this architecture has every instruction complete in one-cycle. In other words, the cycle is the length of time required to complete the longest instruction. So the next logical step is use these stages to divide up instructions into multiple shorter cycles, pipeline the processor, because at any given stage the processor can be using the functional units in the processor simultaneously. This system allows instructions to be completed every cycle in the case where there are no dependences. Realistically the instructions aren't going to be dependence free and they may not be in an ideal order so we get stalls where nothing is happening in a particular functional unit. So we need instruction reordering to help hide latencies
