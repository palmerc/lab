...
To determine how much TLP is available we need to relax all constraints that are not germane. So naturally we will assume unlimited computing resources like registers and memory, we will make all instructions take one cycle, and assume perfect memory disambiguation. This will leave us with only the control-flow constraints imposed by the natural ordering of instructions created by true dependences, and conditional branches.

We will use the SPEC CPU2006 benchmarks and extract traces of execution. From these traces we can use different threading strategies like turning every basic block into a thread and see if any performance improvement is attained.
