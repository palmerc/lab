/* $Header: /fs2/p6.arch.sw-sprt/sw-sprt/proj/spec95/old-working-jpeg/jpeg-5/RCS/spec_image.c,v 1.8 1994/12/28 23:13:51 glew Exp glew $ */
...
{
    if( image-> buffer ) {
	ppm_freearray( (pixel**) image->buffer, image->image_height);
    }
    image->buffer = 0;
		 ...
		 )
{
    ...
    subimage->buffer =
	(JSAMPARRAY) malloc( subimage->image_height * sizeof(JSAMPROW) );
