...
*           3.00 Modifications to work with uCOS-II 2.80, 30 May 2006,
*		 made OSTaskIdleHook() and OSTaskCreateHook() instead of OSTCBInitHook() user callable.
*
*********************************************************************************************************
*/
#define OS_PORT_VERSION 300					//Version number of the uCOS-II LINUX port
...
INT32S eventType = 0;
sigset_t interruptSignalSet, scheduleSignalSet, oldScheduleSignalSet, oldInterruptSignalSet;
BOOLEAN  interruptSignalFlag=FALSE, scheduleSignalFlag=FALSE;
...
//    	pause();
	sigprocmask(SIG_BLOCK, &scheduleSignalSet, &oldScheduleSignalSet);
	while(!scheduleSignalFlag)
	    sigsuspend(&oldScheduleSignalSet);
 	sigprocmask(SIG_UNBLOCK, &scheduleSignalSet, NULL);
    	scheduleSignalFlag=FALSE;
...
//    	pause();
	sigprocmask(SIG_BLOCK, &interruptSignalSet, &oldInterruptSignalSet);
	while(!interruptSignalFlag)
	    sigsuspend(&oldInterruptSignalSet);
 	sigprocmask(SIG_UNBLOCK, &interruptSignalSet, NULL);
    	interruptSignalFlag=FALSE;
...
{   int i;
    ...
    sigemptyset(&scheduleSignalSet);
    sigaddset(&scheduleSignalSet, hScheduleEvent);
...
{   if (idleTrigger)						//Issue a debug message, each time the idle task is reinvoked
    ...
    OS_SLEEP();							//Give Linux a chance to run other applications to when uCOS-II idles
}
...
//      sched_yield();
    ...
    printf("ERROR: Primary thread killed - exiting  %d\n");
}
