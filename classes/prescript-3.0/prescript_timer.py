...
def profread(fn):
    global frags
    frags = io.readFragments(fn)
    
def asdoc():
    global frags, document
...
if __name__ == '__main__':
   ...
   # Real work begins
   global frags, document
   prof = hotshot.Profile("readfragments.prof")
   prof.runcall(profread, inputFilename)
   prof.close()
   
   prof = hotshot.Profile("assembledoc.prof")
   prof.runcall(asdoc)
   prof.close()
   
   prof = hotshot.Profile("dostuff.prof")
   #   del frags # Uncomment this if you are low on memory
   prof.runcall(dostuff, inputFilename, outFilename, format)
   prof.close()
