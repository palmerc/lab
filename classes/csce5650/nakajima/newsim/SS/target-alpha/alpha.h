...
typedef quad_t md_gpr_t[MD_NUM_IREGS];

/* floating point register file entry type */
typedef union {
  quad_t q[MD_NUM_FREGS];	/* integer quad view */
  dfloat_t d[MD_NUM_FREGS];	/* double-prediction floating point view */
} md_fpr_t;

/* control register file contents */
typedef struct {
  quad_t fpcr;			/* floating point condition codes */
  quad_t uniq;			/* process-unique register */
} md_ctrl_t;
...
#define MD_EXO_TO_IREG(EXO, REGS, IDX)					\
  ((REGS)->regs_R[IDX] = (quad_t)(EXO)->as_integer.val)

#define MD_EXO_TO_FREG(EXO, REGS, IDX)					\
  ((REGS)->regs_F.q[IDX] = (quad_t)(EXO)->as_integer.val)
