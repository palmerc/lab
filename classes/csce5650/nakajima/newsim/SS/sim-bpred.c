...
/* skip number of inst's to execute */
static unsigned int skip_insts;

/* debug for trace */
static int debugpipe;
...
"  Predictor `comb' combines a bimodal and a 2-level predictor.\n"
  ...
  /* pipe write skip */
  opt_reg_uint(odb, "-fastfwd", "skip number of inst's to execute",
	       &skip_insts, /* default */0,
	       /* print */TRUE, /* format */NULL);
  /* pipe write debug */
  opt_reg_flag(odb, "-debugpipe", "enable pipe write debug ",
	       &debugpipe, /* default */FALSE, /* !print */FALSE, NULL);
...
#define SYSCALL(INST)							\
  {									\
    if(regs.regs_R[2]==SS_SYS_exit){					\
	fprintf(stderr,"addr_wrong = %d\n",addr_wrong);			\
	fprintf(stderr,"double_regs_count = %d\n",double_regs_count);	\
	fprintf(stderr,"m_size_wrong = %d\n",m_size_wrong);		\
        fflush(stderr);							\
    }									\
...
sim_main(void)
{
  ...
  int v_in1,v_in2,v_in3;
  int v_out1, v_out2;                    /* register values */
  int vl_out1, vl_out2, vl_in1, vl_in2, vl_in3;
  md_addr_t addr_dispatch;              /* access_address */
  int m_size,m_size_old;                           /* access_size */
  FILE *stream;
  md_addr_t predicted_PC;
  unsigned addr_wrong=0,m_size_wrong=0;
  unsigned double_regs_count=0;
  unsigned class_ibu;
  
#if 0
  char *inst_type[]={
    "noop","branch","load","store","other"
    };
#endif
...
#include "operand.def"
      ...
      if (MD_OP_FLAGS(op) & F_CTRL)
	{
      ...
      pipe_data.address = 0;
      pipe_data.brn_type = 0;
      pipe_data.double_regs = 0;
	...
	{
	  /*	  if (MD_OP_FLAGS(op) & F_DW) addr -= 4;*/
	  if ((class_ibu & F_DW) && ((in1 != 0) || (out1 != 0))){
	    double_regs_count ++;
	    pipe_data.double_regs = 1;
	  }
	  if (class_ibu & F_DW) addr -= 4;
	  pipe_data.address = addr;
	  if(addr_dispatch != addr){
	    fprintf(stderr,"pc:%x addr_dispatch = %x addr = %x\n"
		    ,regs.regs_PC ,addr_dispatch, addr);
	    fflush(stderr);
	    addr_wrong++;
	  }
	  if(m_size != m_size_old){
	    fprintf(stderr,"pc:%x m_size = %d m_size_old = %d\n"
		    ,regs.regs_PC ,m_size, m_size_old);
	    fflush(stderr);
	    m_size_wrong++;
	  }
	  if (MD_OP_FLAGS(op) & F_STORE)
	    ...
	    {
	      fprintf(stderr, "donna F_MEM meirei?\n");
	      fprintf(stderr, "pc=%x\n", regs.regs_PC);
	      exit(-1);
	    }
	}
      else if (MD_OP_FLAGS(op) & F_CTRL)
	{
	  ...
	  else if ((MD_OP_FLAGS(op) & (F_UNCOND|F_DIRJMP)) == (F_UNCOND|F_DIRJMP))
	  ...
	  else if ((MD_OP_FLAGS(op) & (F_COND|F_DIRJMP)) == (F_COND|F_DIRJMP))
	    {
	      pipe_data.brn_type = CON_DIRE;
	      pipe_data.address = regs.regs_NPC;
	    }
	  else
	    {
	      pipe_data.brn_type = CR_MISSMATCH;
	      fprintf(stderr, "CR_MISSMATCH\n");
	      fprintf(stderr, "pc=%x\n", regs.regs_PC);
	      exit(-1);
	    }
	}
      ...
      if (!(MD_OP_FLAGS(op) & F_STORE))
	{
	  if (in3 != 0)
		...
		{
		  in1 = in3;
		  fprintf(stderr, "in3->in1\n");
		}
	      else if (in2 == 0)
		{
		  in2 = in3;
		  fprintf(stderr, "in3->in2\n");
		}
	      else
		{
		  fprintf(stderr, "okasii nanndeka siranked\n");
		  exit(2);
		}
	    }
	}
      ...
      if (out1 == 65)
	{
	  ...
	  */
      ...
      pipe_data.sourceC = in3;
      pipe_data.destination = out1;
      pipe_data.pc = regs.regs_PC;       
      pipe_data.m_size = m_size;
      pipe_data.v_sourceA = v_in1;
      pipe_data.v_sourceB = v_in2;
      pipe_data.v_sourceC = v_in3;
      pipe_data.v_destination = v_out1;
      

      /*fprintf(stderr,"%d\n",(int)pipe_data.func_unit);
      ...
      fprintf(stderr,"%d",
	      pipe_data.sourceA);
      fprintf(stderr,"(%d)",
	      reg[pipe_data.sourceA]);
      fprintf(stderr,"%d",
	      pipe_data.sourceB);
      fprintf(stderr,"(%d)",
	      reg[pipe_data.sourceB]);
      fprintf(stderr," inst:%d",
	      (int)pipe_data.func_unit);
      fprintf(stderr," %d pc:%x\n",
	      pipe_data.destination,
	      pipe_data.pc);*/


      /*
      if(pipe_data.destination!=0){
	reg[pipe_data.destination]=sim_num_insn;
      }
      */
      /*
      fprintf(stderr,"%x %2d %2d %2d %2d %1d %1d %d %x\n",
	      pipe_data.pc,
	      (int)pipe_data.sourceA,
	      (int)pipe_data.sourceB,
	      (int)pipe_data.sourceC,
	      (int)pipe_data.destination,
	      (int)pipe_data.func_unit,
	      (int)pipe_data.brn_type,
	      (int)pipe_data.m_size,
	      pipe_data.address
	      );
	      */
      if (sim_num_insn>skip_insts){
	if (debugpipe){
	  fprintf(stderr,"%4d:%x:%s:%2d(%8x),%2d(%8x),%2d(%8x) %2d(%8x)add:%8x\n",
		  (int)sim_num_insn-(int)skip_insts,
		  (int)pipe_data.pc,
		  name_Funit_Type[(int)pipe_data.func_unit],
		  (int)pipe_data.sourceA,
		  (int)pipe_data.v_sourceA,
		  (int)pipe_data.sourceB,
		  (int)pipe_data.v_sourceB,
		  (int)pipe_data.sourceC,
		  (int)pipe_data.v_sourceC,
		  (int)pipe_data.destination,
		  (int)pipe_data.v_destination,
		  (int)pipe_data.address
		  );
	}
	write(18, &pipe_data, sizeof(struct Pip_Inst_Record));
      }
