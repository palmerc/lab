...
quad_t
eval_as_quad(struct eval_value_t val)
{
  switch (val.type)
    {
    case et_double:
...
squad_t
eval_as_squad(struct eval_value_t val)
{
  switch (val.type)
    {
    case et_double:
...
constant(struct eval_state_t *es)	/* expression evaluator */
{
  ...
  errno = 0;
  squad_val = myatosq(es->tok_buf, &endp, /* parse base */0);
  if (!errno && !*endp)
    {
      /* good conversion */
      val.type = et_squad;
  ...
  errno = 0;
  quad_val = myatoq(es->tok_buf, &endp, /* parse base */0);
  if (!errno && !*endp)
    {
      /* good conversion */
      val.type = et_quad;
...
#ifdef HOST_HAS_QUAD
    case et_quad:
      myfprintf(stream, "%lu [quad_t]", val.value.as_quad);
      break;
    case et_squad:
      myfprintf(stream, "%ld [squad_t]", val.value.as_squad);
      break;
#endif /* HOST_HAS_QUAD */
    case et_addr:
