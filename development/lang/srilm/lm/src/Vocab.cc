...
static char RcsId[] = "@(#)$Header: /home/srilm/devel/lm/src/RCS/Vocab.cc,v 1.41 2006/09/18 03:05:11 stolcke Exp $";
#endif
...
{
    VocabIter viter(*this, true);	// output sorted for faster read-back
    VocabIndex index;

    VocabString word;
    while (word = viter.next(index)) {
	fprintf(file, "%u %s\n", index, word);
    }
    fprintf(file, ".\n");
}
