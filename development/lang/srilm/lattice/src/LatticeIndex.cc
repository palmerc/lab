...
static char RcsId[] = "@(#)$Header: /home/srilm/devel/lattice/src/RCS/LatticeIndex.cc,v 1.6 2006/09/13 18:07:46 stolcke Exp $";
#endif
				    ...
				    LHash<const NBestWordInfo *, Prob> &counts)
{
	...
	if (mincount == 0.0 || *count > mincount) {
	    fprintf(file, "%s", lat.getName());
	    fprintf(file, ngram[0].valid() ? " %.2f" : " ?",
			(float)ngram[0].start);
	    fprintf(file, ngram[len-1].valid() ? " %.2f" : " ?",
			(float)(ngram[len-1].start + ngram[len-1].duration));
	    fprintf(file, " %lg", (double)*count);

	    for (unsigned i = 0; i < len; i ++) {
		fprintf(file, " %s", lat.vocab.getWord(ngram[i].word));
	    }
	    fprintf(file, "\n");
	}
    }
